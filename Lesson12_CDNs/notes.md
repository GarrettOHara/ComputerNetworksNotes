...
**First**, with the modern Internet, users are located all over the globe. No matter where a single, massive data center is placed, there’s a potentially vast geographic distance between the users and the data center. That’s a problem because the server-to-client packets must traverse many communication links between many ISPs. If just one of those links has a throughput lower than the video playout rate, there will be annoying interruptions and delays for the user.

**Second**, what happens when a video goes viral? Think of popular movie trailers or important breaking news clips. Popular videos result in a spike in demand and many requests for the exact same video clip and, thus, the same data. It’s wasteful for a single massive data center to repeatedly be sending the exact same data over the same communication link over and over again. Not only does this waste bandwidth, but the Internet video company also loses a lot of money, as they have to pay their ISP for every byte of data transmitted.
...
![Enter Deep](./ImagesLesson12/2.png)
...
![CDN Server 2](./ImagesLesson12/6.png)
...
![What metric should be used for cluster selection?](./ImagesLesson12/9.png)
...
![Consistent Hashing 2](./ImagesLesson12/14.png)
...
![Recursive Queries in DNS](./ImagesLesson12/19.png)
